...
def signal_pe_model():
    ...
    if comb_pe_model:    
   	...
   	model.add(Conv2D(64, filtsize, strides=2, input_shape=(2,n_pix), padding='valid'))
  	...
  	model.add(Dropout(0.5))
    	...
    	model.add(Conv2D(128, filtsize, strides=2))
        if act=='prelu': model.add(PReLU())
        elif act=='leakyrelu': model.add(LeakyReLU(alpha=0.2))
        else: model.add(Activation(act))
        if batchnorm==True: model.add(BatchNormalization(momentum=momentum))

    	model.add(Conv2D(256, filtsize, strides=2))
        if act=='prelu': model.add(PReLU())
        elif act=='leakyrelu': model.add(LeakyReLU(alpha=0.2))
        else: model.add(Activation(act))
        if batchnorm==True: model.add(BatchNormalization(momentum=momentum))

    	model.add(Conv2D(512, filtsize, strides=2))
        if act=='prelu': model.add(PReLU())
        elif act=='leakyrelu': model.add(LeakyReLU(alpha=0.2))
        else: model.add(Activation(act))
        if batchnorm==True: model.add(BatchNormalization(momentum=momentum))
    ...
    if do_pe and not do_only_old_pe_model or retrain_pe_mod:
	    ...
	    signal_batch_pars = signal_train_pars[idx]
            ...
            if ((i % 5000 == 0) & (i>0)):
                ...
                signal_pe.save('best_models/generator.h5', True)   
		...
		# plot true vs predicted values for all training data
                ...
                pe_samples = generator.predict(np.reshape(signal_train_images[idx], (signal_train_images[idx].shape[0],signal_train_images[idx].shape[1],1)))


                #pe_mesg = "%d: [PE loss: %f, acc: %f]" % (i, pe_loss[0], pe_loss[1])
                #print(pe_mesg)
